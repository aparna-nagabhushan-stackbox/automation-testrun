...
`generation.rawCode` gets the same secret masking as the cleaned code (see Data Masking), because a promoted block's code is readable by any logged-in user via `GET /api/blocks`. Masking rewrites value arguments, not selector or locator arguments, so the extracted key sequence is normally unchanged — but that is verified per recording rather than assumed: if masking would alter the extracted sequence (a masked value's text also appearing inside a selector or locator), the unmasked recording is stored instead, so matching stays correct, and a warning is logged.
